...
De una vez, desactivo el firewall. Esto porque generalmente, mi computadora personal suele encontrarse en situaciones en las cuales necesito desplegar servicios de manera rápida que queden a la escucha en alguno de mis puertos. Es un lío modificarlo sobre la marcha, así que prefiero cuidarme mucho.
...
#+begin_src nix :tangle ../nixOS/configuration.nix :noweb tangle
  users.users.mrtaichi = {
    isNormalUser = true;
    extraGroups = [ "wheel" "libvirtd" <<userGroups>>]; # Enable ‘sudo’ for the user. Y VMs
    packages = with pkgs; [
      <<userPackages>>
    ];
    shell = pkgs.zsh;
...
El intended debería de ser que cada lenguaje de programación tenga su propia sección en [[Lenguajes de programación]], y con ello, su propio flujo de trabajo curado y asentado. Pero por ahora, dejaré varios lenguajes aquí sueltos sólo para no perder capacidades en mi IDE, mientras creo los flujos de trabajo correspondientes para cada idioma.
...
- Finalmente, también agrego una configuración que hace que, al abrir los archivos =.org=, sus títulos y subtítulos aparezcan primero con sus contenidos colapsados. De esta manera puedo moverme entre ellos de manera mucho más rápida que si viera todos los títulos y su contenido de un solo vistazo de buenas a primeras.
...
Una parte necesaria de mi flujo de trabajo dentro de Emacs, sobre todo para elaboración de reportes y tareas, es la capacidad de poder pegar imágenes directamente dentro de los documentos =.org=. Particularmente, capturas de pantalla que tenga guardadas en el portapapeles.

Para esto, voy a usar un paquete llamado =org-download= que, acompañado con algunos tweaks, cumple exactamente con esta función.

Primero agregamos el paquete a =packages.el=

#+begin_src lisp :eval never :tangle ../doomEmacs/packages.el
(package! org-download)
#+end_src

Y agregamos la configuración necesaria a mi =config.el=

#+begin_src lisp :tangle ../doomEmacs/config.el :eval never
(after! org-download
...
Estas lineas de configuración solamente hacen estos ajustes:

- =(after! org-download ...)=
Nos asegura que estas configuraciones se apliquen únicamente después de que el paquete org-download haya sido cargado, evitando errores al iniciar Emacs.

- =(setq org-download-method 'directory)=
Establece que las imágenes guardadas se colocarán en un directorio físico dentro de tu sistema de archivos (en lugar de otros métodos de almacenamiento).

- =(setq org-download-image-dir ...)=
Define la ruta de la carpeta donde se guardarán las imágenes de forma automática:

Toma el nombre de tu archivo de notas actual y le quita la extensión (file-name-sans-extension). Le añade el sufijo -img.

Ejemplo: Si tu documento se llama notas.org, se creará automáticamente una carpeta llamada notas-img/ en la misma ruta para almacenar todas las imágenes de ese archivo.

- =(setq org-download-image-org-width 600)=
Le indica a Org-mode que reescale visualmente la imagen a un ancho de 600 píxeles dentro de Emacs (insertando el atributo #+ATTR_ORG: :width 600 en el enlace).

- =(setq org-download-link-format "[[file:%s]]\n"=
     =org-download-abbreviate-filename-function #'file-relative-name)=
    org-download-link-format: Define la sintaxis de Org-mode para insertar la imagen ([[file:ruta/a/imagen.png]]).
    org-download-abbreviate-filename-function #'file-relative-name: Fuerza a que la ruta de la imagen sea relativa al archivo .org en lugar de guardar una ruta absoluta (ej. notas-img/foto.png en vez de /home/usuario/documentos/notas-img/foto.png). Esto permite mover la carpeta de tus notas sin romper las imágenes.

- =(setq org-download-link-format-function ...)=

Restaura o asegura el uso de la función por defecto para dar formato al enlace de la imagen, garantizando compatibilidad con las variables configuradas en el paso anterior.
...
Esta es mi integración para linea de comandos y mi agente autónomo para proyectos que busco realizar puramente con IA, o que busco que salgan en sprints extremadamente rápidos. Es mi alternativa Open Source a tener Claude Code, con miras a en un futuro autohostear los modelos de lenguaje que utilice.
...
En mis primeros semestres de universidad llegué a caer bajo las ideas de que "Python no es un lenguaje de programación real" y pasé a evitarlo como la peste durante un rato. Todo cambió cuando, gracias a Dios, mi lóbulo frontal se terminó de desarrollar y me dí cuenta de que no tiene sentido que existan guerras de lenguajes siempre que nunca endiosemos ni odiemos conscientemente ningún lenguaje de programación y los utilicemos como lo que son: herramientas para realizar un trabajo, con sus propios casos de uso y sus ventajas y desventajas.
...
Luego agregro a mi =.zshrc= (véase [[OhMyZSH][OhMyZSH]]) algunas líneas que vienen también en su página. Las cuales son las siguientes:
...
- Una máquina virtual aislada pero también configurada de manera declarativa para correr análsis de malware aislado y para otros casos de uso en general que también requieran ese aislamiento que solamente una máquina virtual debidamente configurada puede proporcionar.

Vamos a comenzar con la primera feature:

** Suite de herramientas

Usando como punto de partida el proyecto de [[https://github.com/fabaff/nix-security-box][Nix-Security Box]] de Fabian Affolter, voy a seleccionar un conjunto de herramientas particular para ciberseguridad, dividido en categorías. Todas las declararé como paquetes de usuario.
...
Para aplicativos en los que necesite una granularidad mucho más fina de lo que tenga que realizarle a mi sistema operativo. Para este aspecto de mi flujo de trabajo en ciberseguridad, busco un sistema operativo que voy a poder hacer y deshacer a mi gusto, pues esencialmente lo que busco para esta parte del flujo de trabajo es modificarlo de manera extremadamente rápida para la situación que lo amerite.

Por esto, y aprovechando las distintas capas de abstracción que existen actualmente, decidí que el mejor aplicativo para cubrir esta parte de mi flujo de trabajo es Distrobox

Distrobox es una herramienta que permite, entre otras cosas, construir máquinas virtuales de manera rápida, orientadas justamente a ejecutar herramientas en un entorno configurado, y de una manera bastante seamless. Por ejemplo, configura las VMs para que consuman el servidor gráfico y de sonido de la máquina host; lo cual permite que las aplicaciones de dentro de la VM puedan ser lanzadas dentro del entorno de escritorio de la máquina host. Pero en realidad, *Distrobox es una capa de abstracción sobre Podman*, así que habremos de instalar ambas herramientas.

Para comenzar, agrego las siguientes líneas a mi =configuration.nix=, las cuales declaran la virtualización usando Podman.
...
Agrego mi usuario al grupo de podman, para poder correr los comandos necesarios sin problemas.

#+begin_src nix :noweb-ref userGroups
"podman"
#+end_src

Y finalmente, el paquete de sistema de Distrobox.

#+begin_src nix :noweb-ref systemPackages
distrobox
#+end_src

Hecho esto, mi sistema está preparado para crear contenedores con Kali Linux y ahí crear todas las herramientas necesarias. Debido a que los contenedores me serán extremadamente volátiles y, por lo dicho anteriormente, su naturaleza será extremada imperativa y situacional a lo que necesite en este momento, los únicos comandos que guardo aquí sobre cómo interactuar con mi contenedor de ciberseguridad serán los siguientes:

*** Flujo de trabajo con el contenedor de Kali Linux
...
Para finalizar mi flujo de interacción con el contenedor de ciberseguridad que pueda crear con Distrobox, voy a agregar algunas cosillas a la calidad de vida de mi flujo de trabajo.

La primera es para crear una distinción entre la shell de mi equipo host y mis contenedores. Debido a que Distrobox y Podman montan la partición /home dentro del contenedor por default, esto significa que todas mis configuraciones de shell se trasladan a los contenedores. Y debido a que el hostname de los contenedores es idéntico al de la máquina host, esto resulta en que la shell del interior del contenedor y la del exterior resulten idénticas.

Esto me podría traer problemas muy serios en un futuro, al no poder distinguir si estoy dentro de mi contenedor o no y correr comandos que afecten a uno u otro de manera negativa por este error. Por eso, agrego la siguiente verificación a mi =.zshrc= para que cambie de color al entrar al contenedor. Si detecta la variable de entorno adecuada.
