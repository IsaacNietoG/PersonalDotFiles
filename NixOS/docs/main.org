...
Para mi suite de herramientas voy a usar el trabajo realizado por Fabian Affolter en su [[https://github.com/fabaff/nix-security-box][Nix-Security Box]], la cual es un conjunto de herramientas de ciberseguridad para NixOS divididas en categorías. Estas vienen declaradas en varios archivos .nix que, parafraseando sus instrucciones de uso oficiales, se pueden usar de la siguiente manera:

Una vez tenemos el repo disponible en alguna ubicación dentro de nuestro sistema, incluimos la categoría que queramos dentro de nuestro configuration.nix
...
Pero la manera nix que me gusta más y que lo hace completamente declarativo es la siguiente:

Dentro de nuestro flake.nix, declaramos el repositorio de Fabian como input.

Y lo agregamos dentro de las outputs de flake.nix


Luego, en los modules de nuestra flake, declaramos las categorías que vamos a necesitar, siendo:

- proxies, pues ZAP es mi herramienta predilecta para cualquier cosa que responda solicitudes HTTP
- port-scanners, por nmap y rustscan
- web por prácticamente todo
- generic idem
- services, porque contiene muchas herramientas para escanear servicios, particularmente sqlmap, ssh-audit; y además aquí está metasploit
- password, ya que, aunque no le doy tanta prioridad a crackear contraseñas, siempre existe el caso en el CTF de turno
- exploits, aquí está searchsploit
- fuzzers, el nombre lo dice todo
