...
\label{sec:org1854d04}

Mi equipo de trabajo solamente tiene un usuario interactivo: el mío.

No hay mucho que definir aquí excepto tal vez el preludio a \ref{sec:org02d3de0} al definir que voy a usar zsh como emulador de terminal.
...
Comenzamos por agregar la ruta de los binarios de \ref{sec:orgfe5a103} la cual es por lo general:
...
El intended debería de ser que cada lenguaje de programación tenga su propia sección en \ref{sec:org9e15492}, y con ello, su propio flujo de trabajo curado y asentado. Pero por ahora, dejaré varios lenguajes aquí sueltos sólo para no perder capacidades en mi IDE, mientras creo los flujos de trabajo correspondientes para cada idioma.
...
\item Finalmente, también agrego una configuración que hace que, al abrir los archivos \texttt{.org}, sus títulos y subtítulos aparezcan primero con sus contenidos colapsados. De esta manera puedo moverme entre ellos de manera mucho más rápida que si viera todos los títulos y su contenido de un solo vistazo de buenas a primeras.
...
Finalmente, agregamos las líneas correspondientes a activar el \texttt{package} Org Edna. La función de este paquete la explico en la sección de \ref{sec:orge378285}
...
\subsection{Pegar imágenes del portapapeles}
\label{sec:org9e9a0f7}

Una parte necesaria de mi flujo de trabajo dentro de Emacs, sobre todo para elaboración de reportes y tareas, es la capacidad de poder pegar imágenes directamente dentro de los documentos \texttt{.org}. Particularmente, capturas de pantalla que tenga guardadas en el portapapeles.

Para esto, voy a usar un paquete llamado \texttt{org-download} que, acompañado con algunos tweaks, cumple exactamente con esta función.

Primero agregamos el paquete a \texttt{packages.el}

\begin{verbatim}
(package! org-download)
\end{verbatim}

Y agregamos la configuración necesaria a mi \texttt{config.el}
...
\begin{itemize}
\item \texttt{(setq org-download-method 'directory)}
\end{itemize}
...
\begin{itemize}
\item \texttt{(setq org-download-image-dir ...)}
\end{itemize}
...
\begin{itemize}
\item \texttt{(setq org-download-image-org-width 600)}
\end{itemize}
Le indica a Org-mode que reescale visualmente la imagen a un ancho de 600 píxeles dentro de Emacs (insertando el atributo \#+ATTR\textsubscript{ORG}: :width 600 en el enlace).

\begin{itemize}
\item \texttt{(setq org-download-link-format "[[file:\%s]]\textbackslash{}n"}
 \texttt{org-download-abbreviate-filename-function \#'file-relative-name)}
org-download-link-format: Define la sintaxis de Org-mode para insertar la imagen (\begin{center}
\includegraphics[width=.9\linewidth]{ruta/a/imagen.png}
\end{center}).
...
\item \texttt{(setq org-download-link-format-function ...)}
\end{itemize}
...
\subsubsection{}
\label{sec:org11279ef}
\section{Dropbox}
\label{sec:org946a79d}

Utilizo Dropbox muy seguido para mantener la sincronización de mi agendaORG
\subsection{Instalación}
\label{sec:orgdc8ae28}

\begin{verbatim}
maestral
\end{verbatim}
\section{Virtual Box}
\label{sec:orgb5b2de2}
\begin{verbatim}
...
\end{verbatim}
\section{Zen Browser}
\label{sec:org71894fe}

\subsection{Instalación}
\label{sec:org22a59c5}
...
El resto de la configuración inicial es desde la interfaz (TUI) de la herramienta. Consiste en primicia en especificarle qué proveedor de IA utilizar (en mi caso Groq), especificarle la API key para este uso, y seleccionar el modelo. El asistente de instalación se inicia con \texttt{opencode} y luego con \texttt{/connect}
\subsection{Copilot.el}
\label{sec:org4a7c904}
...
Luego agregro a mi \texttt{.zshrc} (véase \hyperref[sec:org02d3de0]{OhMyZSH}) algunas líneas que vienen también en su página. Las cuales son las siguientes:
...
Esta es una suerte de sección legacy porque en realidad desde que uso Virtual Envs y Pyenv he dejado de usar Anaconda, pero creo que no sobra tampoco. Agregamos las siguientes líneas a mi \texttt{.zshrc}
...
\subsubsection{Flujo de trabajo con el contenedor de Kali Linux}
\label{sec:org386050c}

\begin{enumerate}
\item Creación de contenedor
\label{sec:org667a923}

\begin{verbatim}
distrobox create --name kali --image docker.io/kalilinux/kali-rolling
\end{verbatim}
\item Entrar al contenedor
\label{sec:org4137845}

\begin{verbatim}
distrobox enter kali
\end{verbatim}
\end{enumerate}
\subsubsection{QoL (Calidad de vida)}
\label{sec:org4b34123}
